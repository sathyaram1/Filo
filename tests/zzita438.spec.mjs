  ...
  return `<!doctype html><html><body style="padding:20px;font:18px sans-serif">
    ...
    <script>
      ...
      function api(host, ed) {
        ...
        host.edText = () => ed.textContent.replace(/\u00a0/g, ' ');
...
async function setup(app, openTab, testServer) {
  ...
  await app.evaluate(({ session }, url) => {
    ...
    ses.setSpellCheckerLanguages(['it', 'en-US']);
  }, `http://127.0.0.1:${port}/`);
  const page = await testServer.openReady(openTab, pageHtml());
  await page.waitForTimeout(14000); // dizionario pronto
  return { page, srv };
}
...
for (const [label, sel] of CASES) {
  test(`correzione applicata nel campo ricco — ${label}`, async ({ app, openTab, testServer }) => {
    ...
    try {
      await typeInto(page, sel, 'ciiao come stai');
      expect(await ev(page, sel, 'edText')).toBe('ciiao come stai');

      const r = await rightClickWord(page, sel, 0);
      expect(r.word).toBe('ciiao');
      ...
      expect(['ciao','chiao']).toContain(st.firstLabel);
      ...
      expect(after, 'la parola deve essere corretta davvero').toMatch(/^(ciao|chiao) come stai$/);
      ...
      expect(after2.includes('come stai'), 'il fuoco resta nel campo').toBe(true);
