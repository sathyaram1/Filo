...
test('dettatura dal vivo: quel che dico finisce nel campo', async () => {
  ...
  const userData = mkdtempSync(join(tmpdir(), 'filo-zz4-'));
  ...
  const server = createServer((req, res) => {
    ...
    res.end('<html lang="it"><body style="font:16px sans-serif;padding:40px"><textarea id="campo" rows="5" cols="60"></textarea></body></html>');
  });
  ...
  try {
    ...
    console.log('PROVVISORIO:', JSON.stringify(live));
    await pill.click();
    await page.waitForTimeout(25000);
    const val = await page.locator('#campo').inputValue();
    console.log('CAMPO:', JSON.stringify(val));
