...
test('due percorsi della stessa sessione non escono insieme, nemmeno dopo che l’app è stata chiusa a lungo', async () => {
  ...
  const domini = scritture.map((s) => s.body.data.domain).sort();
...
test('l’ordine di uscita non rifà l’ordine della sessione', async () => {
  ...
  assert.equal(scritture.length, 1);
  assert.equal(scritture[0].body.data.domain, 'secondo-esempio.it');
});

test('nel documento che parte non c’è nessuna ora, e nessuna data: la mette il server', async () => {
  await raccogli('https://esempio.it/x', 'una cosa');
  await Collector.flush({ now: Date.now() + RITARDO_MAX_MS + 1000 });
  assert.equal(scritture.length, 1);
  // La data la scrive il server, arrotondata al giorno (contratto della
  // callable, SECURITY.md §8): una data scelta da chi manda decideva anche chi
  // sta in CIMA ai «percorsi già riusciti», e un percorso datato 2099 restava
  // primo per sempre. Qui si tiene fermo che il client non la mandi: non deve
  // poterla scegliere, e un'ora sarebbe la chiave di join che la coda toglie.
  const documento = scritture[0].body.data;
  assert.ok(!('createdAt' in documento), 'la data non la sceglie chi manda');
  const grezzo = JSON.stringify(documento);
  assert.ok(!/\d{4}-\d{2}-\d{2}T/.test(grezzo), 'nel documento non deve viaggiare nessun orario');
});
