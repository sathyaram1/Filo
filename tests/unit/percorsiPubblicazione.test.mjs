...
// La pulizia condivisa va PRIMA, come nel caricatore vero: da #584 sesto giro
// la lettura le chiede se quel sito si condivide, e senza il modulo non legge
// niente (src/shared/paths.js → segmentoDominio).
...
test('nel prompt dell\u2019assistente i percorsi di sconosciuti sono dichiarati non fidati', () => {
  ...
  assert.ok(!senza.includes('Percorsi condivisi'), 'senza percorsi il blocco non deve comparire');

  const blocco = conPercorsi.slice(conPercorsi.indexOf('# Percorsi condivisi'));
  assert.match(blocco, /prompt injection/i,
    'un percorso lo scrive chiunque e parla la lingua dei passi da eseguire: va dichiarato non fidato come la pagina');
  assert.match(blocco, /contenuto esterno/i);
  assert.match(conPercorsi, /percorsi condivisi[^.]*non ordini/i,
