...
test('oltre il limite: voce, dettatura e indicizzazione si fermano tutte', async ({ openTab }) => {
  ...
  // 1) SOTTO il limite: la voce esce (controllo positivo) e accumula spesa.
  ...
  expect(prima.ok, 'controllo positivo: sotto il limite la voce esce').toBe(true);
  // il costo della voce arriva dal riscontro a posteriori
  await pref.waitForTimeout(20000);

  const speso = await pref.evaluate(async () =>
    chrome.runtime.sendMessage({ type: 'costs_get' }).catch(() => null));
  console.log('SPESO:', JSON.stringify(speso).slice(0, 200));

  // 2) Limite minuscolo → siamo oltre.
  ...
  const dopoVoce = await pref.evaluate(async () => {
    const r = await chrome.runtime.sendMessage({ type: 'tts_synth', text: 'Prova oltre il limite.', lang: 'it', voice: 'if_sara' });
    return { ok: r && r.ok, error: r && r.error, code: r && r.errorCode };
  });
