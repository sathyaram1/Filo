...
async function configure(app, theme) {
  await app.evaluate(async (electron, th) => {
...
for (const theme of ['light', 'dark']) {
  test(`blocco di attività leggibile — tema ${theme}`, async ({ app }) => {
    ...
    await expect(page.locator('.dash-bubble-filo').last()).toContainText('accento arancio', { timeout: 30_000 });

    mkdirSync('tests/.shots', { recursive: true });
    await page.screenshot({ path: `tests/.shots/zz-v5-blocco-chiuso-${theme}.png` });
    await page.locator('.dash-activity-head').last().click();
    await expect(page.locator('.dash-activity-body').last()).toBeVisible();
    await page.screenshot({ path: `tests/.shots/zz-v5-blocco-aperto-${theme}.png` });
  });
}
