// DEBUG temporaneo (verifica lettura-pdf): dump di reply dello stub e azioni.
...
test('debug reply stub', async ({ app, shell }) => {
  test.setTimeout(120_000);
  const DIR = mkdtempSync(join(tmpdir(), 'filo-dbg2-pdf-'));
  ...
  await app.evaluate(async ({ percorso }) => {
    ...
    const fake = async ({ attempts, messages, onDelta }) => {
      ...
      globalThis.__vfReplies.push(reply);
  ...
  const dump = await app.evaluate(() => ({
    replies: globalThis.__vfReplies.map((r) => r.slice(0, 500)),
  }));
  console.log('REPLIES:', JSON.stringify(dump.replies, null, 2));
  const bubbles = await page.evaluate(() => [...document.querySelectorAll('.dash-bubble-filo')].map((b) => b.textContent.slice(0, 150)));
