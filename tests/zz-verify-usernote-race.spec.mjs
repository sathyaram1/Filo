  ...
  await page.evaluate(() => {
    ...
    window.filo.message = (msg) => {
      if (msg && msg.type === 'feedback_update' && Object.prototype.hasOwnProperty.call(msg, 'userNote')) {
        ...
        return new Promise((resolve, reject) => { window.__pend.push({ resolve, reject }); });
      }
      return orig(msg);
    };
...
// Scrive nella casella come farebbe l'owner (l'evento input deve partire).
...
test('due invii sullo stesso feedback, risposte in ordine invertito', async ({ openTab }) => {
  ...
  // e la pagina non deve dire "Nessuna modifica" su un testo mai arrivato:
  // ri-salvare "primo" (superato) deve RIPARTIRE davvero
  await scrivi(page, 'primo');
  await btn(page).click();
  await settle(page);
...
test('invio + uscita + rientro + secondo invio (tastiera e bottone)', async ({ openTab }) => {
  const page = await openTab(URL);
  await prepara(page, [fbA({ userNote: 'x0' }), fbB()]);

  await open(page, 'fb-a');
  await scrivi(page, 'x1');
  await box(page).press('Enter');       // invio da tastiera
  await settle(page);

  await open(page, 'fb-b');
  await open(page, 'fb-a');
  // qui la casella mostra ancora il vecchio (la risposta non è tornata)
...
test('primo fallisce, secondo riesce', async ({ openTab }) => {
  ...
  await page.evaluate(() => window.__ok(1));    // il secondo (l'ultimo spedito) riesce
  await settle(page);
  await page.evaluate(() => window.__ko(0));    // il primo, superato, fallisce in ritardo
  await settle(page);
...
test('primo riesce, secondo fallisce', async ({ openTab }) => {
  ...
  // l'errore dell'ultimo spedito deve restare visibile
  await expect(msg(page)).toContainText(/rifiutato|Errore/i);
...
test('cancellazione della frase come secondo invio', async ({ openTab }) => {
  ...
  // il secondo invio (svuotamento) DEVE essere partito
  expect(await sent(page)).toEqual([
    { id: 'fb-a', userNote: 'w1' },
    { id: 'fb-a', userNote: '' },
  ]);
