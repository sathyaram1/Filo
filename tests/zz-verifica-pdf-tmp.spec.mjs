...
// NB: il primo parametro di app.evaluate è il modulo Electron, l'argomento è il secondo.
async function stubProvider(app, { percorso, needle, okText, koText }) {
  await app.evaluate(async (_electron, { percorso, needle, okText, koText }) => {
...
test('PDF con testo: il testo estratto entra nel contesto e la risposta arriva in chat', async ({ app, shell }) => {
  ...
  const withDoc = calls.filter((c) => c.includes(MARKER));
  expect(withDoc.length).toBeGreaterThanOrEqual(1);
  expect(withDoc[0]).toContain('4321,99');
  expect(withDoc[0]).toContain('Fine del documento');
});
...
test('PDF scansione senza testo: il contesto dichiara la scansione, la chat lo dice', async ({ app, shell }) => {
  ...
  // E nel contesto c'è la consegna esplicita di non inventare.
  const calls = await app.evaluate(() => globalThis.__vfCalls);
  const declared = calls.filter((c) => c.includes('nessun testo estraibile'));
  expect(declared.length).toBeGreaterThanOrEqual(1);
  expect(declared[0]).toContain('NON inventare');
});
...
test('sveglie a parole: ricorrente creata, visibile coi giorni, spostata e cancellata', async ({ app, openTab }) => {
...
test('changelog con voce PDF + voci sveglie; manifesto con la capacità documenti', async ({ app }) => {
  const chk = await app.evaluate(() => {
    ...
    const capsFlat = JSON.stringify(globalThis.SN_CAPABILITIES ? Object.values(globalThis.SN_CAPABILITIES) : []);
    return {
      version: top.version,
