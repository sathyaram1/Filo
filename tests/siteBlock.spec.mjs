...
// #590 (settimo giro) — IL DIVIETO VALE ANCHE SUI FILE.
// Un sito che l'utente ha messo in lista non deve arrivargli addosso da nessuna
// strada, e scaricare è una strada: il tasto destro su un link o su un'immagine
// offre di prendere quello che c'è dall'altra parte, e quel link ce l'ha messo
// la pagina, non l'utente. I byte di un'immagine li prende il main per conto
// suo, fuori dalla sessione della scheda, quindi non incontrano nemmeno il
// filtro delle richieste: senza il controllo il file arriva.
//
// Asserisce il comportamento visibile: niente file sul disco e la stessa
// notifica "Sito bloccato" del blocco di una scheda.
test('#590 scaricare da un sito della lista non parte, e lo dice', async ({ app, shell, openTab, testServer }) => {
  await enableBlock(shell);
  ...
  const porta = fileServer.address().port;

  try {
    const page = await testServer.openReady(openTab, `<!doctype html><html><body style="padding:40px">
      <a id="file" href="http://${BLOCKED_HOST}:${porta}/manuale.pdf">Il manuale (PDF)</a>
      <p><img id="foto" src="http://${BLOCKED_HOST}:${porta}/foto.png" width="40" height="40" /></p>
    </body></html>`);
    const dir = await app.evaluate(() => process.env.FILO_DOWNLOAD_DIR);
    expect(dir).toBeTruthy();

    // 1) il file puntato da un link.
    await page.locator('#file').click({ button: 'right' });
    await page.locator('.sn-menu button', { hasText: 'Salva file' }).click();
    await expect(shell.locator('.shell-notif', { hasText: 'Sito bloccato' }).first())
      .toBeVisible({ timeout: 8000 });

    // 2) l'immagine, che prende un'altra strada (i byte li scarica il main).
    await shell.evaluate(() => document.querySelectorAll('.shell-notif').forEach((n) => n.remove()));
    await page.locator('#foto').click({ button: 'right' });
    await page.locator('.sn-menu button', { hasText: 'Salva immagine' }).first().click();
    await expect(shell.locator('.shell-notif', { hasText: 'Sito bloccato' }).first())
      .toBeVisible({ timeout: 8000 });

    // Niente è finito sul disco, per nessuna delle due strade.
    await shell.evaluate(() => new Promise((r) => setTimeout(r, 1500)));
    const arrivati = existsSync(dir) ? readdirSync(dir) : [];
    expect(arrivati, `dal sito della lista non deve arrivare niente: ${arrivati.join(', ')}`).toEqual([]);
  } finally {
...
test('#590 controllo: da un sito fuori lista lo scaricamento parte', async ({ app, shell, openTab, testServer }) => {
  await enableBlock(shell);
  const FILE = Buffer.from('%PDF-1.4\n% finto pdf #590 fuori lista\n' + 'w'.repeat(512));
  const fileServer = createServer((req, res) => {
    res.writeHead(200, {
      'Content-Type': 'application/pdf',
      'Content-Length': FILE.length,
      'Content-Disposition': 'attachment; filename="fuori-lista.pdf"',
    });
    res.end(FILE);
  });
  await new Promise((r) => fileServer.listen(0, '127.0.0.1', r));
  ...
  try {
    const page = await testServer.openReady(openTab, `<!doctype html><html><body style="padding:40px">
      <a id="file" href="http://127.0.0.1:${porta}/fuori-lista.pdf">Un altro manuale</a>
    </body></html>`);
    await page.locator('#file').click({ button: 'right' });
    await page.locator('.sn-menu button', { hasText: 'Salva file' }).click();
    const dir = await app.evaluate(() => process.env.FILO_DOWNLOAD_DIR);
...
// #590 (nono giro) — L'ECCEZIONE DEL MOTORE DI RICERCA E L'INDIRIZZO CHE UNA
// PAGINA SI RISCRIVE ADDOSSO.
//
// L'eccezione vale per una pagina di RISULTATI, e a dire se una pagina è di
// risultati è il suo indirizzo. Ma dentro la propria origine l'indirizzo lo
// riscrive la pagina, con una riga e senza ricaricare niente: una pagina
// qualunque ospitata sul nome di un motore (sites.google.com è l'esempio da cui
// era nata la stretta del giro 7) si portava nel percorso dei risultati e da lì
// apriva qualunque sito della lista. Senza la correzione questi due test sono
// rossi: il sito della lista si apre e nessuna notifica dice niente.
const OSPITE_MOTORE = 'sites.google.com';

// Un server che serve la pagina "di chiunque" sul nome del motore e la pagina
// del sito in lista. `azione` è quello che la pagina fa dopo essersi riscritta
// l'indirizzo.
async function serverPaginaOspitata(azione) {
  const { createServer } = await import('node:http');
  let porta = 0;
  const server = createServer((req, res) => {
    ...
    res.end('<!doctype html><meta charset="utf-8"><h1 id="o">pagina di chiunque</h1><script>'
      + 'history.replaceState({}, "", "/search?q=qualcosa");'
      + `setTimeout(() => { ${azione(`http://${BLOCKED_HOST}:${porta}/pagina`)} }, 250);`
      + '</script>');
  });
  await new Promise((r) => server.listen(0, '127.0.0.1', r));
  porta = server.address().port;
  return {
    partenza: () => `http://${OSPITE_MOTORE}:${porta}/view/qualcuno`,
    chiudi: async () => {
      try { server.closeAllConnections?.(); } catch (_) {}
      await new Promise((r) => server.close(r));
    },
...
async function sitoInListaAperto(shell) {
  const snap = await shell.evaluate(() => window.filoShell.tabs.snapshot());
  return snap.tabs.some((t) => {
    try { return new URL(t.url).hostname === BLOCKED_HOST; } catch (_) { return false; }
  });
}

test('#590 una pagina ospitata sul nome di un motore non si prende l\'eccezione riscrivendosi il percorso', async ({ shell, openTab }) => {
  await enableBlock(shell);
  const srv = await serverPaginaOspitata((u) => `location.href = ${JSON.stringify(u)};`);
  try {
    const page = await openTab(srv.partenza());
    await page.waitForSelector('#o', { timeout: 8000 });
...
test('#590 la stessa pagina non se lo prende nemmeno aprendo una scheda nuova o un riquadro', async ({ shell, openTab }) => {
  await enableBlock(shell);
  // Scheda nuova e riquadro incorporato chiedono alla stessa decisione della
  // navigazione in-place: se la riscrittura la aprisse, si aprirebbero tutte e
  // tre insieme.
  const scheda = await serverPaginaOspitata((u) => `window.open(${JSON.stringify(u)}, "_blank");`);
  try {
    const page = await openTab(scheda.partenza());
    await page.waitForSelector('#o', { timeout: 8000 });
    await page.waitForTimeout(2000);
    expect(await sitoInListaAperto(shell)).toBe(false);
  } finally {
  ...
  const riquadro = await serverPaginaOspitata((u) => 'const f = document.createElement("iframe"); '
    + `f.src = ${JSON.stringify(u)}; f.width = 900; f.height = 600; document.body.appendChild(f);`);
  try {
    const page = await openTab(riquadro.partenza());
    await page.waitForSelector('#o', { timeout: 8000 });
    await page.waitForTimeout(2000);
    const dentro = page.frames().some((f) => {
      try { return new URL(f.url()).hostname === BLOCKED_HOST && f.url().includes('/pagina'); } catch (_) { return false; }
    });
    expect(dentro).toBe(false);
  } finally {
    await riquadro.chiudi();
  }
});

test('#590 controllo: da una pagina di risultati VERA l\'eccezione vale ancora', async ({ shell, openTab }) => {
  await enableBlock(shell);
  // Stesso banco di prova, ma l'indirizzo dei risultati è quello da cui la
  // pagina è stata davvero caricata: qui l'eccezione deve continuare a valere,
  // altrimenti la correzione avrebbe spento una cosa che funzionava.
  const { createServer } = await import('node:http');
  let porta = 0;
  const server = createServer((req, res) => {
    const host = String(req.headers.host || '').split(':')[0];
    ...
    res.end('<!doctype html><meta charset="utf-8"><h1>risultati</h1>'
      + `<a id="go" href="http://${BLOCKED_HOST}:${porta}/pagina">il sito che cercavi</a>`);
  });
  await new Promise((r) => server.listen(0, '127.0.0.1', r));
  porta = server.address().port;
  try {
    const page = await openTab(`http://${OSPITE_MOTORE}:${porta}/search?q=qualcosa`);
    await page.waitForSelector('#go', { timeout: 8000 });
    await page.evaluate(() => document.getElementById('go').click());
    await page.waitForTimeout(2000);
    expect(await sitoInListaAperto(shell)).toBe(true);
  } finally {
    try { server.closeAllConnections?.(); } catch (_) {}
    await new Promise((r) => server.close(r));
  }
