...
// Il clic si dà DENTRO la pagina (`evaluate`), non col mouse di Playwright:
// le pagine di Filo vivono in WebContentsView e la finestra qui è nascosta,
// quindi un click() di Playwright resta appeso sui controlli di azionabilità.
// È la stessa strada dei giri 1-4.
async function premi(p, id) {
  await p.waitForSelector(`#${id}`, { timeout: 8000 });
