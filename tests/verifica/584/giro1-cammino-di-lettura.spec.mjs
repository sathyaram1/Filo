...
// Prima c'è constants.js, che è il modulo base, poi pathsSafety.js, che è la
// pulizia condivisa col server (#585): paths.js non spedisce niente senza, di
// proposito — un ripiego che manda il percorso così com'è sarebbe la porta di
...
test('i percorsi già riusciti arrivano ancora nel prompt dell’assistente di pagina', async () => {
  ...
  const prompt = g.SN_PATHS_SAFETY.formatKnownPathsForPrompt(letti);
...
test('il documento spedito non porta nessun identificativo del mittente, e nemmeno può portarlo', async () => {
  ...
  g.__fetch = async (_url, opts) => {
    ...
    return { ok: true, json: async () => ({ result: { saved: true, id: 'xyz' } }), text: async () => '' };
  };
  ...
  // Il DOCUMENTO che il server scriverà: niente del mittente. Il clientId
  // viaggia accanto, non dentro — è l'identità su cui il server tiene i limiti
  // di frequenza (#585), e nella raccolta non entra.
  ...
  const testo = JSON.stringify(documento);
  expect(testo).not.toContain('Mozilla');
  // e la data non la sceglie chi manda: la mette il server, arrotondata al
  // giorno (contratto della callable, SECURITY.md §8). Un percorso datato 2099
  // restava in cima ai «già riusciti» per sempre.
