...
// `pathSubmit`, che scrive con l'Admin SDK e bypassa le regole. Quindi, con
// l'emulatore, le prove di scrittura di questo file ora si aspettano un
// RIFIUTO — è il comportamento voluto — e per riempire la raccolta prima di
// provare le letture serve l'Admin SDK, non una POST col client. Le prove di
// LETTURA, che sono il cuore di #584, valgono immutate.
...
for (const [nome, docs] of [['banca-esempio.it', banca], ['clinica-esempio.it', clinica]]) {
  for (const d of docs) {
    console.log(`  ${nome.padEnd(22)} intent="${d.fields.intent}"`);
    console.log(`  ${''.padEnd(22)} createdAt (dal client, arrotondato) = ${d.fields.createdAt}`);
...
for (const [b, c, d] of coppie) {
  console.log(`  "${b.fields.intent}" ↔ "${c.fields.intent}" : ${(d / 1000).toFixed(3)} s`);
}
const [vicino] = coppie;
console.log(`\nRicucitura: il percorso "${vicino[0].fields.intent}" su banca-esempio.it e`);
console.log(`"${vicino[1].fields.intent}" su clinica-esempio.it distano ${(vicino[2] / 1000).toFixed(3)} s:`);
