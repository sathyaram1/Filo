// Ponte: fa girare le prove del giro 5 del #569 anche a chi lancia Playwright.
//
// Stesso motivo dei ponti dei giri precedenti: le prove di questo giro sono
...
// `.spec.mjs`: senza questo ponte le prove del giro 5 non le rilancerebbe
// nessuno.
...
const PROVE = ['tests/verifica/569/giro5-le-porte-fuori-dal-file.test.mjs'];

// Ogni esca fa girare la sentinella intera in un processo a parte.
test.setTimeout(5 * 60 * 1000);

test('#569 giro 5: le prove del giro passano', () => {
  let uscita = '';
  try {
    uscita = execFileSync(
      process.execPath,
    ...
    throw new Error(`le prove del giro 5 del #569 sono rosse:\n${rossi || testo.slice(-2000)}`);
  }
