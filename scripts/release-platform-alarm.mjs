 ...
 */
export const PASSI = {
  checkout: 'il prelievo dello stesso commit costruito per Windows',
  node: 'la preparazione di Node',
  dipendenze: "l'installazione delle dipendenze (npm ci)",
  versione: 'il controllo che qui ci sia lo stesso numero di versione pubblicato per Windows',
 ...
 */
export function componiAllarme({ piattaforma, versione, passo, esecuzione, repo, mancanti } = {}) {
  ...
  const titolo = `Filo per ${nome} non è nella release ${v}${p ? ` (passo «${p}»)` : ' (passo non identificato)'}`;
  ...
  if (conf && repo) {
    righe.push(
      `Cosa vede chi scarica: https://github.com/${repo}/releases/latest/download/${conf.scarica} risponde 404. E chi ha già Filo per ${nome} non riceve questa versione: l'aggiornamento automatico legge ${conf.aggiornamento}, che nella release non c'è.`,
      '',
    );
  }
  ...
  const elenco = String(mancanti || '').trim();
  if (elenco) righe.push(`File attesi e non trovati nella release: ${elenco.split(/\s+/).join(', ')}.`);

  if (esecuzione) righe.push(`Registro dell'esecuzione: ${esecuzione}`);

  righe.push(
    '',
    `Cosa fare: capisci perché quel passo si è fermato e ripubblica i file per ${nome} SU QUELLA STESSA release (${v}). Rilanciare il lavoro di release non basta: decide se pubblicare contando i commit dopo l'ultimo tag, quindi senza commit nuovi non rifà niente e la ${v} resta senza.`,
  );
...
async function main() {
  ...
  const { titolo, testo } = componiAllarme({
    ...
    passo: passoFallito(leggiEsiti(process.env.ESITI)),
