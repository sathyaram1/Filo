// One-time: ottieni il refresh token Firebase dell'account ROBOT per le routine.
//
// PERCHÉ ESISTE
...
//   stampa quel refresh token, da incollare nella variabile d'ambiente della
//   routine `FILO_ROUTINE_REFRESH_TOKEN`.
//
// USA UN ACCOUNT DEDICATO, NON il tuo admin "owner". Il ruolo `routines` è
// volutamente debole: anche se il token trapelasse (la piattaforma routine non
// ha un secret store sicuro), il danno massimo è rimescolare gli stati di
// triage tra todo/done/clarify. Con l'account owner, invece, una fuga sarebbe
// compromissione totale. Crea quindi un Google account robot, fai il login qui
// con QUELLO, e aggiungi la sua email alla collezione `routines` da console.
//
// USO:
//   node scripts/routine-login.mjs
//   (si apre il browser; accedi con l'account robot; il token viene stampato)
...
function openBrowser(url) {
  // Best-effort multipiattaforma. Se fallisce, l'URL è comunque stampato sotto.
    ...
    const server = createServer((req, res) => {
      ...
      res.end('<!doctype html><meta charset="utf-8"><body style="font-family:system-ui;padding:2rem">'
        + (error || !code ? 'Login non riuscito. Puoi chiudere questa scheda.'
          : 'Login routine completato. Torna al terminale: il refresh token è stato stampato.')
        + '</body>');
      server.close();
...
async function main() {
  ...
  console.log('\nApro il browser per il login dell\'account ROBOT.');
  ...
  console.log('\n──────────────────────────────────────────────────────────────');
  console.log('Account robot:', fb.email || '(email non disponibile)');
  console.log('email_verified:', fb.emailVerified);
  console.log('\n1) Aggiungi questa email alla collezione `routines` su Firebase Console');
  console.log('   (documento con ID = l\'email qui sopra).');
  console.log('\n2) Incolla questo refresh token nella variabile d\'ambiente della');
  console.log('   routine `FILO_ROUTINE_REFRESH_TOKEN`:\n');
