...
module.exports = function register(on, ctx) {
  ...
  on(MSG.DEFAULT_MODELS_LIST, async (msg) => {
    try {
      ...
      // Il catalogo è pubblico e la chiave è facoltativa, ma resta una richiesta
      // AL FORNITORE: dal cancello unico come tutte le altre (#591, secondo
      // giro). Scritta a mano qui era la riga da copiare per una chiamata che
      // invece si paga.
      const raw = await Gate.listCatalog({ provider, apiKey }) || [];
      const Caps = globalThis.SN_MODEL_CAPS;
