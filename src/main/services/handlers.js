...
// Interpreta un valore "sì/no" scritto in linguaggio naturale (l'LLM o l'utente
// possono usare molte varianti). Ritorna true/false oppure null se ambiguo.
...
// Mappa le preferenze che Filo può cambiare da chat (azione IMPOSTA_PREFERENZA)
// sull'esatto insieme esposto dalla pagina Preferenze. Ogni voce: sinonimi di
// chiave + un `build(valore)` che valida il valore grezzo e produce
// { partial, label }. `partial` è il pezzo di settings da fondere (deepMerge
// preserva i campi annidati vicini); `label` è la conferma leggibile.
// Solo queste chiavi sono scrivibili: niente apiKeys, provider, ecc.
const PREF_SETTERS = [
  {
