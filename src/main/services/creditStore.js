...
(function (global) {
  ...
  function freshState(today = dateKey(), config) {
    return {
      initialized: true,
      balance: resolveConfig(config).initial,
  ...
  function ensure(state, config) {
    const s = state && state.initialized ? state : freshState(undefined, config);
    ...
    if (typeof s.balance !== 'number') s.balance = resolveConfig(config).initial;
  ...
  function applyRefill(state, today = dateKey(), autoFeedbackEnabled = false, config) {
    ...
    let added = 0;
    if (missed > 0) {
      // Accredita i giorni non ancora accreditati col valore CORRENTE della
      // config: se l'owner ha cambiato dailyRefill, i giorni mancati valgono il
      // NUOVO importo (comportamento naturale, nessuna retroattivita' strana).
  ...
  function rewardForPriority(priority, config) {
    const table = resolveConfig(config).feedbackResolveByPriority || {};
