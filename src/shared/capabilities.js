...
(function (global) {
  ...
  const CAPABILITIES = [
    ...
    // ─────────────────────── Lettura e traduzione ───────────────────────────
    {
      ...
      desc: 'Traduce tutto il testo visibile della pagina — titolo, sommario, didascalie, riquadri laterali, voci di menu e testo dei link, non solo i paragrafi, compreso il testo dentro i componenti con cui sono costruiti i siti moderni — mantenendo impaginazione, link, immagini e illustrazioni; mentre lavora mostra a che punto è arrivata, e puoi tornare all’originale quando vuoi. Traduce anche le scritte che non stanno nel testo: il grigio dentro i campi di ricerca, i suggerimenti che compaiono fermando il mouse, le descrizioni delle immagini e le voci dei menu a tendina. Se il sito aggiunge altro testo dopo, scorrendo o cambiando schermata senza ricaricare, puoi tradurre anche quello senza rifare il resto. Se si interrompe a metà (rete che salta, credito finito) te lo dice, con il motivo e il punto in cui si è fermata, e puoi riprenderla: completa solo i pezzi mancanti senza rifare quelli già tradotti. Le pagine lunghissime le traduce a tratti: te lo dice, e la ripresa arriva in fondo.',
      invoke: 'Menu del tasto destro → "Traduci la pagina"; a traduzione finita la stessa icona diventa "Mostra originale", se si è interrotta diventa "Riprendi traduzione", e se nel frattempo il sito ha aggiunto altro testo diventa "Traduci il testo nuovo" (in entrambi i casi "Mostra originale" resta lì sotto come voce).',
      doesNot: 'Non traduce il testo dentro le immagini, i video, i riquadri di codice, le illustrazioni (grafici, loghi, icone disegnate nella pagina) e le formule, che restano intatte con i loro colori; non tocca quello che scrivi tu nei campi né i valori che il sito rimanda indietro quando invii un modulo; non traduce le pagine dove non trova testo (in quel caso te lo dice). Se un sito tiene chiusi certi suoi componenti, quel testo non è leggibile da nessuno script e resta nella lingua originale: in quel caso l’avviso dice che la pagina è tradotta solo in parte.',
    },
