...
(function (global) {
  ...
  const CAPABILITIES = [
    ...
    {
      id: 'read-web-page', title: 'Filo legge le pagine web', category: 'assistant',
      desc: 'Filo apre una pagina web e ne legge davvero il contenuto, poi ti risponde con quello che c’è scritto: un prezzo, un orario di apertura, un punteggio, una clausola, i numeri di una tabella. Prima sapeva cercare sul web, ma dei risultati vedeva solo il titolo e due righe di riassunto: per qualunque dato che stava dentro la pagina poteva solo tirare a indovinare o dirti di andare a leggere da te. Ora, quando il riassunto non basta, apre le pagine più promettenti e le legge, compreso quello che il sito tiene chiuso finché non lo apri: la risposta di una domanda frequente, la scheda del listino che non è in primo piano, il seguito di «leggi tutto». Se la pagina è già aperta in una tua scheda legge quella, così funziona anche sui siti che si costruiscono da soli e su quelli dove hai già fatto il login. Se l’indirizzo porta a un PDF, gli orari di un comune, una bolletta, un modulo, legge quello come fa coi documenti che hai sul computer, anche quando il sito te lo offre come file da scaricare.',
      invoke: 'Chiedilo a parole all’assistente (nuova scheda): "leggi questa pagina e dimmi quanto costa", "fammi una tabella dei prezzi", oppure incolla l’indirizzo. Dopo una ricerca lo fa da solo quando il riassunto non basta. Lo fa anche l’assistente laterale (Alt+H, Ctrl+Alt+H su Mac) quando cerca sul web per aiutarti su un sito.',
      doesNot: 'Legge e basta: non apre schede, non compila moduli e non clicca niente al posto tuo. Di una pagina lunghissima legge la prima parte e ti avverte dove si è fermato. Non va a bussare da solo agli indirizzi della tua rete privata (il router, un server di casa o dell’ufficio); se quella pagina è già aperta in una tua scheda e sei tu a chiederlo dalla chat, la legge da lì. L’assistente laterale invece legge dalla scheda solo la pagina su cui lo stai usando: per ogni altro indirizzo scarica, e un indirizzo che potrebbe portare fuori tuoi dati te lo fa vedere e aspetta il tuo OK. Non legge i siti segnalati come pericolosi. Il testo di una pagina lo scrive chi possiede quel sito: Filo lo tratta come informazione da riportare, mai come istruzioni da eseguire.',
    },
    {
