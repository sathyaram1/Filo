...
(function (global) {
  ...
  const CAPABILITIES = [
    ...
    // ─────────────────── Immagini e cattura schermo ──────────────────────────
    {
      ...
      desc: 'Descrive un’immagine della pagina, direttamente lì sotto. Quando il file se la porta dietro, sopra la descrizione compare anche l’etichetta di origine. Dice se l’immagine si dichiara generata o modificata con l’AI, oppure scattata con una fotocamera, e chi lo dichiara. Filo legge tre cose scritte dentro il file. Le credenziali firmate (Content Credentials, lo standard C2PA), di cui controlla la firma, chi l’ha messa e se qualcuno ha cambiato il file dopo. L’etichetta «Digital Source Type» dei metadati IPTC/XMP. I parametri di generazione che alcuni programmi scrivono dentro i PNG. Tutto sul tuo computer, senza rete e senza consumare crediti, sugli stessi byte che servono alla descrizione. La stessa lettura vale se lo chiedi a voce o in chat («questa foto è fatta con l’AI?»).',
      ...
      doesNot: 'Non guarda i pixel e non tira a indovinare. Se il file non porta etichette, sull’origine non compare niente, e quel silenzio non vuol dire che l’immagine sia vera: uno screenshot, un salvataggio che ricomprime o il caricamento su un social cancellano le etichette, e molti generatori non le scrivono affatto. Non legge i marchi invisibili, cioè SynthID (Google, e le immagini di ChatGPT), TrustMark di Adobe e quello di Stable Diffusion. Non dà percentuali e non emette verdetti propri. Riporta quello che il file dichiara, e dice quando la firma non è valida, quando è di un ente che non riconosce e quando il file è stato cambiato dopo la firma.',
    },
    {
