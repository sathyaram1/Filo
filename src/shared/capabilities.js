...
(function (global) {
  ...
  const CAPABILITIES = [
    ...
    },
    {
      ...
      desc: 'Quando clicchi un link a un file (PDF, ZIP, allegato) Filo lo scarica mostrando l’avanzamento in tempo reale con un indicatore in alto, tra le schede — barra e percentuale — e a fine scaricamento ti avvisa con "Apri file" e "Apri cartella"; se qualcosa va storto (rete caduta, errore del server, spazio finito) te lo dice invece di restare in silenzio. I file scaricati restano in un elenco che sopravvive alla riapertura di Filo. Un file che il computer ESEGUE (.exe, .msi, .bat, .dmg, .pkg, .sh, .jar…) non entra nella cartella Download da solo: Filo si ferma, ti dice che è un programma e da quale sito arriva, e lo scarica solo se rispondi di sì — finché non rispondi il file non è sul computer. Nell’elenco i programmi portano la scritta «Programma», e «Apri file» su un programma chiede una seconda conferma, perché aprirlo vuol dire eseguirlo. Se il file parte da una scheda aperta apposta dal sito — una scheda bianca o una pagina "il download partirà a breve…" che non hai mai toccato — Filo la chiude da solo appena il download parte e ti riporta alla pagina di partenza, con un avviso "Riapri" per rimetterla se ti serviva.',
      ...
      doesNot: 'Un file preso da un link finisce direttamente nella cartella Download di sistema, senza chiederti dove ogni volta (le immagini e i filmati salvati dal clic destro invece te lo chiedono, e quelli si possono annullare ma non mettere in pausa). L’elenco degli scaricamenti e i suoi comandi restano dentro Filo: i siti che visiti non possono leggerlo né comandarlo. La domanda sui programmi si può spegnere, o togliere per i siti di cui ti fidi, in Sicurezza; Filo non controlla se un programma è davvero pericoloso: ti dice che è un programma e da dove arriva, la scelta resta tua. Le finestre in incognito non ci finiscono.',
    },
    {
