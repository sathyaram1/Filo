...
(function (global) {
  ...
  const CAPABILITIES = [
    ...
    },
    {
      ...
      desc: 'Chiedi a Filo di farti da timer ("timer di 10 minuti per la pasta"): il conto alla rovescia compare in alto nella nuova scheda e, allo scadere, suona un avviso, più una notifica di sistema. La suoneria si sente da qualunque scheda tu stia guardando, e in cima alla finestra compare un pulsante per fermarla. In Preferenze scegli quale motivo suona e quanto forte, a parole o con le manopole. Puoi mettere in pausa un timer e riprenderlo quando vuoi, e puoi chiedere a Filo di annullarlo o di cambiargli la durata invece di rifarlo da capo.',
      invoke: 'Chiedi un timer all’assistente; i timer attivi e quelli che stanno suonando si vedono in alto nella nuova scheda. Quando suona, il pulsante in cima alla finestra la ferma; se stavi guardando una pagina a tutto schermo, Filo ne esce perché tu possa premerlo. Sulla scheda del timer trovi ⏸ per metterlo in pausa e ▶ per riprenderlo. Per toglierlo, la × sulla scheda oppure chiedilo a Filo ("annulla il timer della pasta").',
      doesNot: 'Finché Filo è in esecuzione, suoneria e notifica di sistema arrivano: anche ridotto a icona, e anche a finestra chiusa, dove Filo la riapre per farsi sentire. Con Filo completamente chiuso non suona nulla. Un timer chiesto da una finestra incognito suona in quella finestra e vale solo finché resta aperta, e di lui non arriva nessuna notifica di sistema: il nome che gli hai dato resterebbe scritto fra le notifiche del computer.',
    },
    {
      id: 'filo-alarms', title: 'Sveglie', category: 'assistant',
      desc: 'Chiedi a Filo una sveglia ("mettimi una sveglia alle 7 per lavoro", "sveglia tra 3 ore"): all’orario stabilito suona un avviso, da qualunque scheda tu stia guardando, e arriva una notifica di sistema. Se l’orario è già passato oggi, la sveglia viene messa per domani. Può anche ripetersi: dille quando ("il lunedì e il mercoledì", "nei giorni feriali", "tutte le mattine") e suona a ogni giorno indicato, senza rimetterla ogni volta. Le sveglie già messe le gestisci sempre a parole: cancellarne una, cancellarle tutte, o spostarne l’orario.',
      invoke: 'Chiedi la sveglia all’assistente; le sveglie programmate compaiono in alto nella nuova scheda con il loro orario (e i giorni, se si ripete) e puoi rimuoverle con la ×. Quando una suona, il pulsante in cima alla finestra la ferma; se stavi guardando una pagina a tutto schermo, Filo ne esce perché tu possa premerlo. A parole: "cancella la sveglia della palestra", "leva tutte le sveglie", "sposta quella delle 7 alle 8".',
      doesNot: 'Non suona se Filo è completamente chiuso: il browser deve restare in esecuzione (va bene anche ridotto a icona, o con la finestra chiusa: all\'ora giusta Filo la riapre). Per svegliarti al mattino affidati anche a una sveglia vera. La ripetizione va a giorni della settimana: non sa fare "ogni due settimane" o "il primo del mese". Una sveglia chiesta da una finestra incognito suona in quella finestra e vale solo finché resta aperta, senza notifica di sistema: l\'orario e il nome resterebbero scritti fra le notifiche del computer.',
    },
    {
    ...
    },
    {
      ...
      desc: 'Tema (chiaro/scuro/sistema), dimensione del testo delle pagine interne, archiviazione automatica delle schede, stile dell’assistente, voce/velocità/tono della lettura, suoneria del timer e notifiche (quale motivo, quanto forte, quanto restano). Ogni voce si cambia anche a parole, chiedendola a Filo, e la pagina aperta mostra subito il cambiamento.',
