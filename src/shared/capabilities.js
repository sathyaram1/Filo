...
(function (global) {
  ...
  const CAPABILITIES = [
    ...
    },
    {
      ...
      desc: 'Per usare i modelli con i crediti di Filo serve un codice d’invito: lo riscatti nella pagina Crediti e ricevi i crediti d’ingresso, più una quota ogni giorno che si accumula. Il saldo lo tiene il server: è il tetto di spesa di una chiave OpenRouter creata solo per te. Insieme ai crediti ricevi i tuoi codici d’invito, da dare a chi vuoi. Quando i crediti finiscono Filo te lo dice e non riprova: aspetti quelli del giorno dopo o metti una tua chiave OpenRouter, che ha la precedenza. La chiave tua si mette e si toglie dalla pagina Crediti (o dalle Opzioni: è lo stesso campo), che mostra gli ultimi caratteri, quanto ha speso e quanto le resta. Se OpenRouter la rifiuta (chiave non valida, credito finito, permessi mancanti), Filo risponde lo stesso coi tuoi crediti e lo scrive sotto la risposta. La pagina Crediti ricorda l’ultimo rifiuto finché la chiave non torna a funzionare, e alla chiamata dopo Filo riprova prima la tua chiave.',
