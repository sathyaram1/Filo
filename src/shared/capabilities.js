...
(function (global) {
  ...
  const CAPABILITIES = [
    ...
    },
    {
      ...
      desc: 'L’assistente conversazionale di Filo: gli scrivi a parole cosa ti serve e ti risponde, tenendo conto di ciò che gli hai detto in passato. Sa quali schede hai aperte, e le riconosce dal sito («chiudi quella del giornale»); il titolo della pagina non entra nella conversazione, perché lo sceglie chi ha scritto quella pagina. Conosce anche i tuoi documenti dell’editor (appunti inclusi) tramite un breve riassunto di ciascuno, sempre aggiornato: così sa di cosa parlano senza doverli tenere tutti aperti, e quando per risponderti gli serve un documento per intero lo legge al momento. Oltre a rispondere può fare cose per te — aprire pagine, cercare sul web, impostare timer, prendere appunti, regolare preferenze — chiedendoti conferma per le azioni delicate. Sopra ogni risposta c’è una riga discreta che dice cosa sta facendo mentre lavora («Sta ragionando · …», «Cerco sul web: …») e, a lavoro finito, cosa ha fatto («Ha cercato sul web e letto un documento · 40 s»); un click la apre e mostra tutto, nell’ordine: il ragionamento del modello per intero, le azioni compiute (timer avviato, sveglia impostata, ricerca sul web, lettura di un file) come righe con un’icona, e gli esiti dei comandi.',
    ...
    },
    {
      ...
      desc: 'Filo legge mail, pagine e documenti scritti da altri. Quando poi ti scrive un avviso o una risposta nata da lì, un secondo modello guarda quel testo prima che compaia, e non è lo stesso modello che l\'ha scritto. Se il testo cerca di farti dare credenziali, codici o pagamenti, o di farti aprire un collegamento che porta altrove da dove dice, l\'avviso non compare. Al suo posto trovi una riga che dice cosa ha fermato e da chi veniva. I collegamenti dentro un avviso mostrano sempre il dominio a cui portano davvero, prima che tu li apra, e un indirizzo che Filo ha trovato dentro quello che ha appena letto non si apre da solo: prima te lo mostra e ti chiede se andare. Quello che il controllo ferma resta scritto nel registro «Avvisi fermati» delle Preferenze.',
      invoke: 'Funziona da solo, e vale per tutta la conversazione: se Filo ha letto una pagina, anche le risposte alle domande che fai dopo passano dal controllo. Passa dal controllo anche quello che quel turno lascia scritto per dopo \u2014 il nome di un timer o di una sveglia, un appunto, una cosa che Filo memorizza, lo stile con cui ti scrive \u2014 cos\u00ec una pagina non pu\u00f2 far entrare le proprie frasi nella memoria di Filo e fartele ripetere domani. Passa dal controllo anche il saluto della nuova scheda e i suggerimenti accanto, perch\u00e9 Filo li scrive guardando le pagine che hai salvato, e il titolo di una pagina lo sceglie chi l\'ha scritta; un suggerimento nato cos\u00ec ti chiede conferma prima di aprire un indirizzo. E vale per i tuoi documenti dell\'editor: dentro un documento ci pu\u00f2 stare una mail o una pagina che hai incollato, quindi una risposta nata da un documento passa dal controllo come una nata da una ricerca, sia quando Filo lo legge per intero sia quando gli basta il riassunto che ne ha sempre davanti. Dalla riga di un avviso fermato, e dalla bolla in chat, apri direttamente quello che è stato fermato; il registro sta in Preferenze → «Avvisi fermati». Il modello che fa il controllo si sceglie in Opzioni → Modelli → «Guardiano degli avvisi nati da mail e pagine», e il suo consumo compare nei crediti sotto «Controlli di sicurezza».',
      doesNot: 'Non controlla le risposte a domande che non hanno letto niente di altri: su «che ore sono» non si scomoda. Se il controllo non riesce, perché la rete è giù o il fornitore non risponde, l\'avviso non ti viene mostrato lo stesso. Aspetta in coda, visibile, e compare appena il controllo riesce. Se invece a mancare è il modello del controllo, Filo te lo dice e ti manda dove si imposta: aspettare, in quel caso, non servirebbe. Lo stesso vale se hai acceso «solo modelli a pesi aperti» e il controllo finirebbe sullo stesso modello che scrive le risposte: Filo si ferma e te lo dice, invece di far controllare un testo a chi l\'ha scritto.',
    },
    {
