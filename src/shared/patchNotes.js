...
(function (global) {
  ...
  const NOTES = [
    // ↓ Nuove versioni in cima.
    {
      ...
      features: [
        'Quando Filo legge qualcosa scritto da altri, per esempio una pagina web, un documento, dei risultati di ricerca, i titoli delle schede che hai aperto o quello che gli stampa un comando, da lì in poi può fare solo le cose che gli avevi chiesto. Se gli serve fare altro te lo chiede, dicendoti quale azione e perché, e il tuo sì vale per quella richiesta e basta: il messaggio dopo, se gli serve ancora, te lo richiede, e un permesso dato all’assistente di un sito non lo segue su un altro sito. Quello che prova a fare e non può te lo scrive in chat invece di lasciarti credere che l’abbia fatto. Così una pagina che prova a dargli ordini non ottiene niente. Due cose non te le chiede nemmeno. Dopo aver letto non si scrive regole in memoria e non cambia lo stile con cui ti parla, perché restano per sempre e valgono in ogni conversazione: quello che ha trovato leggendo te lo salva come appunto. E una conversazione riparte sempre con gli stessi limiti che aveva: se l’hai lasciata aperta a lungo, se l’intervista di benvenuto ricompare in un’altra scheda o dopo un riavvio, o se un turno era caduto a metà e premi Riprova. Vale anche per un’immagine che gli alleghi, che l’hai scelta tu ma l’ha scritta qualcun altro: Filo fissa l’elenco prima di guardarla. E il bottone che ti mette in chat per aprire un file apre solo file del tuo computer, mai un indirizzo, e ti dice quale file prima che tu lo prema. In filo://security/ vedi, per ogni richiesta, cosa Filo ha letto, cosa gli era permesso e cosa gli è stato impedito; l’elenco resta anche dopo aver chiuso e riaperto Filo.',
        'I bottoni della nuova scheda mandano in chat quello che c’è scritto sopra, e passandoci sopra col mouse vedi dove portano prima di premerli.',
        'Anche un collegamento dentro una risposta di Filo dice dove porta: ci passi sopra col mouse e leggi l’indirizzo intero. Se Filo l’ha scritto dopo aver letto una pagina o un documento, ti ci manda solo se è un indirizzo dove è stato davvero, per esempio un risultato della ricerca che ha appena fatto; per qualunque altro te lo chiede prima, mettendoti l’indirizzo sotto gli occhi.',
        'E un documento che Filo ti legge sta sul tuo computer: un percorso che punta a un computer in rete non lo apre più. Su Windows aprirlo avrebbe spedito le tue credenziali all’altro capo, e quel percorso lo può aver scelto una pagina che Filo aveva appena letto.',
        'Una ricerca non si porta più via quello che Filo ha appena letto: se la domanda che sta per mandare al motore di ricerca contiene un tuo dato, per esempio un numero preso dal documento che ti stava leggendo, te la fa leggere intera prima che parta. E un collegamento scritto da Filo dentro una pagina web, per esempio nel riquadro «Spiega» o nell’assistente Aiuto, si apre passando dallo stesso controllo della chat invece di aprirsi da solo.',
        'Lo stesso controllo vale anche per una pagina che Filo apre da sé: se l’indirizzo si porta dietro un tuo dato preso dal documento che ti stava leggendo, prima te lo fa leggere intero. E un collegamento dentro una risposta passa dal controllo con qualunque gesto tu usi per aprirlo, tasto centrale compreso: prima quel gesto lo apriva senza chiedere niente.',
        'Quando Filo cambia l’aspetto di una pagina, quello che ci mette dentro non va più a prendere immagini o altro in rete: era l’ultima strada per cui quello che ti aveva appena letto poteva uscire dal computer senza che nessuno ti chiedesse niente. E la ricerca che l’assistente dentro una pagina fa sul testo che gli indichi ora ti mostra l’indirizzo intero prima di partire: prima ne vedevi l’inizio e ne partiva molto di più.',
      ...
      fixes: [
        ...
        'Il bottone con cui Filo ti propone di aprire un file adesso lo apre davvero: prima non succedeva niente.',
        'Se chiedi a Filo di segnarti un appuntamento, adesso l’appuntamento esiste davvero: in chat compare un bottone che lo aggiunge al calendario del tuo computer. Prima Filo scriveva «Evento creato» e non c’era nessun evento da nessuna parte.',
        ...
        'Filo sa di nuovo quali schede hai aperte: se glielo chiedi te le elenca, e quando sono tante la nuova scheda ti propone di fare pulizia. Prima rispondeva sempre che non ne avevi nessuna.',
        ...
        'Quando l\'assistente ti aiuta a fare qualcosa su un sito, il percorso che Filo condivide con le altre installazioni non dice più chi l\'ha fatto: niente identificativo, niente browser, niente ora, e dall\'indirizzo della pagina spariscono email, numeri di conto e nome utente. Su quello che resta decide un modello, e il nome del sito lo vede anche lui. Se ci riconosce una persona, il percorso non parte. Filo lo tiene sul tuo computer e lo manda più tardi, a un\'ora sorteggiata.',
        'E da quello che è solo tuo Filo non condivide niente: il router di casa, un disco di rete, l\'intranet dell\'ufficio, un server di prova, un indirizzo che non è un sito pubblico. Lì quei passi non servirebbero a nessun altro, e il nome dell\'indirizzo direbbe come si chiama la tua macchina o per chi lavori. Su quei siti l\'assistente non ti chiede nemmeno se ha funzionato.',
        'Chi legge i percorsi condivisi può chiederli solo un sito per volta. Prima bastava una richiesta per scaricarli tutti e rimettere in fila quelli della stessa persona su siti diversi.',
        'Sotto la domanda «Ha funzionato?» ora c\'è scritto cosa succede se rispondi: i passi di quel percorso aiutano chi userà Filo sullo stesso sito.',
