...
(function (global) {
  ...
  const PROMPTS = {
    ...
    filoChat: ({ profilo, preferenze, espansioni, lezioni, stato, history }) =>
      ...
      `COMANDO DA TERMINALE ("lancia ls", "fai git status", "installa le dipendenze con npm install", "crea la cartella build") → emetti l'azione ESEGUI_COMANDO con {comando} = il comando shell esatto. NON inventare un livello di sicurezza né chiedere conferma a parole: è il SISTEMA a classificare il comando e a decidere se eseguirlo subito (sola lettura), chiedere conferma (modifiche recuperabili) o richiedere di digitare "conferma" (cancellazioni / comandi non riconosciuti). L'output ti torna mostrato in chat. Richiede la modalità terminale attiva: se è spenta il sistema te lo segnala da sé — allora proponi di attivarla (IMPOSTA_PREFERENZA modalita_terminale true). UN comando per azione, niente concatenazioni con && o ; (vengono trattate al massimo attrito).\n` +
      `APRIRE DA UN ALTRO PAESE ("apri questa scheda dalla Francia", "fammi vedere questo sito dagli Stati Uniti", "questo contenuto è bloccato in Italia") → emetti APRI_DA_PAESE con {paese} = codice ISO a due lettere (Francia→fr, Stati Uniti→us, Regno Unito→gb, Germania→de, Giappone→jp…). Instrada la scheda web attiva attraverso un IP di quel paese. NOTA: la scheda così aperta non condivide i login con le altre (è isolata). Per riportarla in Italia ("torna in italia", "togli il paese") usa RIMUOVI_PROXY; per riportarle TUTTE usa RIMUOVI_PROXY con tutte:true ("chiudi/togli tutte le schede aperte da un altro paese").\n` +
      `REGOLA PERSISTENTE PER SITO ("questo sito sempre dagli USA", "apri sempre netflix dagli Stati Uniti", "d'ora in poi X dalla Germania") → emetti REGOLA_PROXY_DOMINIO con {paese} (codice ISO) e, se l'utente nomina un sito diverso da quello che ha davanti, {dominio} = il dominio (es. "netflix.com"); se dice "questo sito" lascia {dominio} vuoto, lo risolve il sistema dalla scheda attiva. La regola PERSISTE al riavvio: ogni volta che si apre quel dominio la scheda nasce già aperta da quel paese. Per togliere la regola ("togli la regola sugli USA per questo sito", "non aprire più X dall'estero") emetti REGOLA_PROXY_DOMINIO con rimuovi:true (e {dominio} se serve).\n\n` +
      `═══ AZIONI DISPONIBILI ═══\n` +
  ...
  const DEFAULT_SETTINGS = {
    ...
    // - domainRules: istruzioni persistenti per dominio ("questo sito sempre
    //   dagli USA"). Mappa { "<host>": { country, tier } }: alla nascita di una
    //   tab su quel dominio (o un sottodominio) la tab parte già proxata. È una
    //   REPLACE_KEY (vedi storage.js) così rimuovere una regola la cancella.
    proxy: {
      datacenter: '',
      residential: '',
      bypass: '',
