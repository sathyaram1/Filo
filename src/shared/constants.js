...
(function (global) {
  ...
  const PROMPTS = {
    ...
    filoChat: ({ profilo, preferenze, espansioni, lezioni, stato, history, modelName }) =>
      ...
      `  • accent (colore d'accento, da cui ereditano link e selezione) · text (colore del testo) · background (sfondo) · topbar (barra in alto del browser: la fascia dietro le schede — è QUESTO per "colora la barra in alto/la barra delle schede", NON \`background\` né \`colore_tab\`) · muted (testo secondario) · border (bordi) · error (colore degli errori)\n` +
