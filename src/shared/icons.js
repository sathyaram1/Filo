// Libreria di icone SVG (guida: src/styles/ICONS.md): viewBox 24×24, disegno entro ~20×20,
// tratto 1.75 round, outline puro su `currentColor`. Ritorna una stringa <svg> che i
// consumer iniettano via innerHTML: non contiene input utente, quindi è sicuro.
...
(function (global) {
  ...
  // Logo Filo: una «f» corsiva a tratto unico, pensata per evocare un filo che si annoda.
  const filoLogo =
    `<path d="M14.6 5.4c-1.2-1-3-.6-3.6 1.3L7.4 18c-.5 1.7-1.9 2.2-2.9 1.4"/>` +
    `<path d="M6.2 11.6h7.3"/>`;

  // Zoom: espandi a tutto schermo.
  const zoom =
    `<path d="M9 4H4v5"/>` +
    `<path d="M4 4l5 5"/>` +
    `<path d="M15 4h5v5"/>` +
  ...
  // Screenshot: mirino a quattro angoli con cerchio centrale.
  const screenshot =
  ...
  // Screenshot parziale: lo stesso mirino con un rettangolo tratteggiato (selezione).
  ...
  // Trascrivi: foglio con righe di testo e una «T» che evoca l'OCR.
  const transcribe =
  ...
  // Immagine: cornice, sole in alto a sinistra e due montagne in un unico polyline da bordo
  // a bordo (niente segmenti scollegati).
  const image =
  ...
  // Traduzione: «文» in alto a sinistra e «A» in basso a destra, entrambi come path.
  // Niente freccia: la sola disposizione comunica la trasformazione.
  const translate =
  ...
  // Opzioni: ingranaggio a 6 denti, raggi 9 (tip) e 6.5 (base), half-angle 12.5°/17.5°.
  const options =
  ...
  // Pipetta: cap rettangolare perpendicolare all'asse, corpo diagonale fino al tip.
  const colorPicker =
    `<path d="M14 4l6 6l-3 3l-2-2l-9 9h-3v-3l9-9l-1-1z"/>`;

  // Lucchetto: voce «Sicurezza» del menu Impostazioni.
  const lock =
  ...
  // Incognito: tesa del cappello e due lenti tonde. Menu Impostazioni e menu secondario.
  const incognito =
    `<path d="M3 13h18"/>` +
  ...
  // Modelli AI: tre nodi connessi (rete neurale), da distinguere a occhio dall'ingranaggio.
  const models =
  ...
  // QR code: i tre occhi agli angoli più qualche modulo, senza disegnarlo per intero.
  const qrCode =
  ...
  // Interrompi lettura: stesso altoparlante con una «X» al posto delle onde.
  const stopReading =
    `<path d="M4 9.5v5h3l4.5 3.5v-12L7 9.5H4z"/>` +
    `<path d="M15.5 10l4 4"/>` +
    `<path d="M19.5 10l-4 4"/>`;

  // Cronologia: orologio con freccia antioraria e lancette.
  const history =
    `<path d="M3.5 12a8.5 8.5 0 1 1 2.6 6.1"/>` +
    `<path d="M3.5 18v-4h4"/>` +
    `<path d="M12 7.5V12l3 2"/>`;

  // Crediti: moneta forata cinese con un FILO che passa nel foro e risale in un cappio —
  // i due simboli del prodotto insieme.
  const credits =
  ...
  // Red-team: scudo con un mirino al centro (l'attacco che prova a forare le difese).
  const redteam =
  ...
  // Mazzi: due carte a ventaglio, il gesto di sventagliare una mano.
  const decks =
    `<rect x="8" y="5" width="10" height="14" rx="1.5"/>` +
    `<path d="M6.5 7.2l-2.9.8 3.1 11 3.4-.9"/>`;

  // Appunti: foglio con l'angolo piegato e due righe di testo.
  const note =
  ...
  // Ogni icona è una FUNZIONE (size) => stringa SVG: i consumer chiedono taglie diverse
  // (16 per la riga, 20 per la griglia) senza rifare il wrapper a mano.
  const ICONS = {
