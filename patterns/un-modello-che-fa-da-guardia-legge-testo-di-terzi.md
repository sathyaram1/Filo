...
- `src/shared/pathsSafety.js` — `redactSelector` e `neutralizzaMarcature`: alla
  fonte, prima che il testo venga anche pubblicato, e di nuovo in lettura dentro
  `formatKnownPathsForPrompt`, che è la parte che già lo faceva.
- `src/main/services/pathsCollector.js` — `sanitizeUserMessages`.
