...
**Cosa succede quando lo accendi.** Quasi tutte le funzioni di Filo nascono con un modello proprietario: se l'interruttore si limitasse a spegnerlo, spegnerebbe mezza app. Quindi ogni funzione **passa automaticamente all'equivalente a pesi aperti**, e la pagina delle opzioni ti dice, appena lo accendi, quante cambiano e su quali modelli finiscono — spegnerlo rimette tutto com'era. Le poche che un equivalente non ce l'hanno — oggi la lettura ad alta voce, la dettatura e l'indicizzazione dell'archivio, mestieri che nessun modello a pesi aperti raggiungibile da Filo sa fare — **si fermano e te lo dicono**, nominando la funzione. Il criterio è il mestiere, non l'etichetta: il sostituto deve saper fare *quel* lavoro, non solo avere i pesi aperti. Le funzioni che devono guardare un'immagine passano quindi a un modello aperto che le immagini le legge, e non a quello di solo testo che basta al resto; mettere la dettatura su un modello che legge solo testo non sarebbe una sostituzione, sarebbe consegnarti una funzione rotta senza dirtelo.
